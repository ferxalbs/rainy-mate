...
- **Recurring runs can now be edited natively instead of forcing delete-and-recreate** — existing playbooks and prompt-based scheduled tasks can now be updated through the desktop runtime, the workspace surface, and the agent tool contract:
  - `src-tauri/src/services/persistent_scheduler.rs`, `src-tauri/src/commands/workspace.rs`, `src-tauri/src/lib.rs` — added canonical scheduled-run update plumbing, next-run recalculation on edit, and invoke-surface wiring
  - `src-tauri/src/services/skill_executor/args.rs`, `src-tauri/src/services/skill_executor/registry.rs`, `src-tauri/src/services/skill_executor/scheduler.rs`, `src-tauri/src/services/tool_policy.rs`, `src-tauri/src/services/local_agent_security.rs`, `src-tauri/src/ai/agent/specialist.rs`, `src/components/agent-chat/neural-config.ts` — added `update_recurring_task` with explicit policy coverage and agent/runtime visibility
  - `src/components/workspace/WorkspaceRecurringRuns.tsx`, `src/services/tauri.ts` — added native recurring-run edit controls for both playbook and prompt tasks from the workspace UI
...
- **Native recurring scheduling is now robust across the default-agent, Launchpad-policy, and macOS-runtime layers** — scheduling now behaves like a first-class governed workspace primitive instead of a partially wired feature:
  - `src-tauri/src/services/default_agent_spec.rs`, `src-tauri/src/ai/agent/manager.rs`, `src-tauri/src/commands/agent.rs` — built-in default-agent specs now refresh instead of fossilizing older tool visibility/instruction state
  - `src-tauri/src/services/mate_launchpad.rs` — Launchpad allowlists now preserve recurring-task control tools so pack-constrained workspaces do not accidentally hide scheduler capabilities
  - `src-tauri/src/services/persistent_scheduler.rs`, `src-tauri/macos/RainyAutoLaunch.swift`, `src-tauri/src/services/macos_auto_launch.rs`, `src-tauri/build.rs`, `scripts/build-macos-bridges.sh`, `src-tauri/tauri.conf.json`, `src-tauri/src/services/settings.rs`, `src-tauri/src/commands/settings.rs`, `src/components/settings/tabs/PermissionsTab.tsx`, `src/services/tauri.ts`, `src-tauri/src/services/mod.rs`, `src-tauri/src/lib.rs` — added launch-at-login control, immediate scheduler startup polling, and native scheduled-run notifications on macOS
...
- **Default-agent recurring-task responses no longer regress into fake shell follow-up after native scheduling succeeds** — once the workspace scheduler has accepted a task, the assistant now describes the real stored state instead of inventing `chmod`, `crontab`, or invalid tool-as-shell guidance:
  - `src-tauri/src/services/skill_executor/scheduler.rs`, `src-tauri/src/commands/agent.rs` — tightened post-scheduling tool guidance so successful native scheduling stays in the native mental model and uses `list_recurring_tasks` / `update_recurring_task` correctly
...
- **Workspace Launchpad for first-party MaTE scenarios** — the desktop now includes a guided launch surface for workspace-first packs and first-run scenarios, backed by Rust state instead of frontend-only presets:
  - `src-tauri/src/services/mate_launchpad.rs` — new launchpad domain service defining workspace trust presets, first-party packs, first-run scenarios, prompt construction, and launch-result recording
  - `src-tauri/src/services/workspace.rs` — workspace records now persist launchpad state and can be materialized directly from an active workspace path
  - `src-tauri/src/commands/workspace.rs` — new commands for listing pack definitions, listing first-run scenarios, reading launchpad state, updating launch config, building first-run prompts, and recording launch results
  - `src-tauri/src/services/mod.rs`, `src-tauri/src/lib.rs` — launchpad services and commands wired into the desktop startup/invoke surface
  - `src/components/workspace/WorkspaceLaunchpad.tsx`, `src/components/workspace/index.ts`, `src/components/layout/AppSidebar.tsx`, `src/App.tsx`, `src/services/tauri.ts`, `src/types/workspace.ts` — new launchpad UI, sidebar entry, Tauri bindings, and app-level scenario handoff into chat
...
- **Chat hydration and launch handoff are hardened against stale scope and history overwrite races** — optimistic/live messages now survive thread hydration and launch requests wait for the correct thread state before execution:
  - `src/hooks/useAgentChat.ts` — `ensureChatScope()` now prefers the externally selected thread when it differs from stale internal state; history hydration merges persisted history with optimistic/live messages instead of replacing them; `runNativeAgent()` returns structured run results for launch result recording
  - `src/components/agent-chat/AgentChatPanel.tsx` — pending launch execution waits until hydration is complete before starting a guided run

- **Model selection is again fully dynamic and no longer falls back silently to Gemini** — chat runs and guided launches now resolve the persisted selected model and refuse to substitute a hardcoded fallback when the selector has not hydrated yet:
  - `src/components/agent-chat/AgentChatPanel.tsx` — removed hardcoded `"gemini-3-flash-preview"` launch/submit fallback, added selected-model resolution via `getSelectedModel()`, and delayed guided launches until the real selected model is available
  - `src/services/settings.rs` — restored the settings default path so the system remains aligned with the existing selector-driven model flow instead of being pinned to a single Rainy model

- **Guided launch deduplication now survives React remounts** — the same scenario request can no longer be consumed twice and produce two concurrent assistant runs:
  - `src/App.tsx`, `src/components/agent-chat/AgentChatPanel.tsx` — launch request IDs are now consumed exactly once at the app boundary before a run starts

- **Gemini tool schemas are normalized to the provider’s accepted schema subset** — local JSON Schema constructs generated by `schemars` are now inlined/removed before Gemini function declarations are sent:
  - `src-tauri/src/ai/providers/gemini_adapter.rs` — local `#/definitions/...` refs are resolved before emission, `definitions` and `$ref` are stripped during Gemini schema cleanup, and focused regression tests now cover ref-based tool schemas
...
- `pnpm exec tsc --noEmit` → pass
- `cd src-tauri && cargo check -q` → pass
- `cd src-tauri && cargo test gemini_adapter -- --nocapture` → pass
...
- **GitHub Actions macOS universal verification step looking for the wrong bundle artifact** — release/CI verification now checks the real Tauri app bundle executable path instead of searching for a non-existent standalone file named `Rainy MaTE`:
  - `.github/workflows/publish.yml` — universal binary verification now resolves `Rainy MaTE.app/Contents/MacOS/rainy-mate`
  - `.github/workflows/ci.yml` — same universal binary verification fix for the main CI workflow
