...
pub async fn save_agent_spec(app_handle: AppHandle, spec: AgentSpec) -> Result<String, String> {
    ...
    tokio::fs::create_dir_all(&dir).await.map_err(|e| format!("Failed to create spec dir: {}", e))?;
    ...
    tokio::fs::write(&path, body).await.map_err(|e| format!("Failed to write spec file: {}", e))?;
...
pub async fn list_agent_specs(app_handle: AppHandle) -> Result<Vec<AgentSpec>, String> {
    ...
    let mut entries = tokio::fs::read_dir(&dir).await.map_err(|e| format!("Failed to list specs: {}", e))?;
    while let Some(entry) = entries.next_entry().await.map_err(|e| format!("Failed to read spec entry: {}", e))? {
        let path = entry.path();
