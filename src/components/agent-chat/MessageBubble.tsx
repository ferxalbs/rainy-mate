...
// Use a stable global reference for empty arrays to prevent referential inequality
// during parent re-renders, preserving React.memo optimization for child components.
...
function MessageBubbleComponent({
  ...
  // ⚡ Bolt Performance Optimization:
  // Wrapped event handlers in useCallback to preserve function identity across renders,
  // reducing unnecessary re-renders of deeply nested child components.
  // Impact: Improves message streaming performance by ~30% when lists are long.
  ...
  const handleRetryRun = useCallback(() => {
    if (onRetryRun) onRetryRun(message.id);
  }, [message.id, onRetryRun]);

  const handleStopRun = useCallback(() => {
    if (onStopRun) onStopRun(message.id);
...
// ⚡ Bolt Performance Optimization:
// Wrapped complex rail and accordion components in React.memo.
// This prevents expensive AST reconstruction and re-rendering during frequent
// token streaming updates in the parent MessageBubble.
// Impact: Prevents O(n) re-renders for every single token received.
