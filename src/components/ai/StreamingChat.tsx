...
import type { RoutedChatRequest } from "../../services/tauri";
...
export function StreamingChat() {
  ...
  const handleSend = async () => {
    ...
    // Construct routed request
    const request: RoutedChatRequest = {
    ...
    try {
      await streamWithRouting(request);
    } catch (err) {
        ...
        >
          ...
          {providers.map((provider) => (
            <option key={provider.id} value={provider.id}>
              Force Provider: {provider.id} ({provider.provider_type})
            </option>
          ))}
        </select>
      </div>
              ...
              <div className="flex justify-start">
                <div className="max-w-[70%] rounded-lg p-3 bg-white border border-gray-200">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {(streamedProviderId || streamedModel) && (
      ...
      {isStreaming && (
        <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <div className="text-sm text-gray-600 grid grid-cols-2 gap-4">
            ...
            <div>Length: {fullText.length} chars</div>
            {streamedProviderId && (
              <div>
                Provider:{" "}
                <span className="font-mono">{streamedProviderId}</span>
              </div>
            )}
            {streamedModel && (
              <div>
                Model: <span className="font-mono">{streamedModel}</span>
              </div>
            )}
          </div>
        </div>
      ...
      {chunks.length > 0 && !isStreaming && (
        <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
          ...
          <div className="text-sm text-gray-600 grid grid-cols-2 gap-4">
            ...
            <div>Final length: {fullText.length} chars</div>
            {streamedProviderId && (
              <div>
                Provider:{" "}
                <span className="font-mono">{streamedProviderId}</span>
