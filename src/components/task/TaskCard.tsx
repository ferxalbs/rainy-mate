...
const statusIcons = {
    queued: <Timer className="size-4 text-orange-500 shrink-0" />,
    running: <Loader2 className="size-4 text-blue-500 animate-spin shrink-0" />,
    paused: <Pause className="size-4 text-yellow-500 shrink-0" />,
    completed: <CheckCircle2 className="size-4 text-green-500 shrink-0" />,
    failed: <XCircle className="size-4 text-red-500 shrink-0" />,
    cancelled: <Square className="size-4 text-muted-foreground shrink-0" />,
};
...
export function TaskCard({ task, onPause, onStop, onViewDetails }: TaskCardProps) {
    ...
    return (
        <div className="floating-card p-4 animate-task-appear hover-elevate">
            {/* Header */}
            ...
            {/* Error message */}
            {task.error && (
                <p className="text-xs text-red-500 bg-red-500/10 p-2 rounded-lg mb-3">{task.error}</p>
            )}
            ...
            {isActive && (
                <div className="flex flex-wrap gap-1.5 pt-2 border-t border-border/50">
                    ...
                    >
                        ...
                        <span className="ml-1">{task.status === "paused" ? "Resume" : "Pause"}</span>
                    </Button>
                    <Button
                        size="sm"
                        variant="danger"
